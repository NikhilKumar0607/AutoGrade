...
.progress-bar.yellow { background: #ffc107; color: black; }
...
def home():
    ...
    if request.method == "POST":
        ...
        if essay:
            ...
            <b>Excepted Similarity:</b> {round(similarity,2)}%
            <div class="progress">
        ...
        # ---------------- CODE BLOCK ----------------
            ...
            if feedback["status"] != "ok":
                ...
                test_score = evaluate_code(code, test_cases)
                quality_score = code_quality_score(code)
                final_score = 0.7 * test_score + 0.3 * quality_score

                result = (
                    f"<b>Final Code Score:</b> {round(final_score, 2)} / 10<br><br>"
                    f"<b>Test Case Score:</b> {round(test_score, 2)} / 10<br>"
                    f"<b>Code Quality Score:</b> {round(quality_score, 2)} / 10<br><br>"
                    f"<b>Code Feedback:</b><br>{feedback['message']}"
                )
